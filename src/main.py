#!/usr/bin/env python3
"""
Datasheet Parser CLI - Unified

Extract pin data from electronic component datasheets and generate schematic symbols.
Supports both standard mode and --layout-mode for Vision API layout extraction.
"""
...
def main():
    ...
    if args.verbose:
        ...
        mode = "Layout Mode (LLM + Vision)" if args.layout_mode else "Standard Mode (LLM only)"
        print(f"Mode: {mode}")
    ...
    try:
        ...
        if args.verbose:
            print("\n[3/4] Extracting pin data with LLM...")

        if not api_key:
            print("Error: API key required for pin data extraction")
            print("Set --api-key, DATASHEET_PARSER_API_KEY, or FASTCHAT_API_KEY environment variable")
            sys.exit(1)
        ...
        if args.verbose:
            ...
            if args.verbose and len(pin_data.pins) > 0:
        ...
        if args.layout_mode:
            if args.verbose:
                print("\n[Extra] Extracting layout structure with Vision API...")
            ...
            if layout_page:
                ...
                if args.verbose and layout_text:
        ...
        if args.verbose:
            print("\n[4/4] Validating package and generating schematic...")
