"""
Comprehensive Pin Extraction Test
...
EXPECTED_RESULTS = {
    ...
    "pdfs/MC74HC595A.PDF": {
        ...
        "pin_count": 16,
        "expected_pins": [
            {"number": 1, "name": "QB"},  # OUTPUT_15
            {"number": 2, "name": "QC"},  # OUTPUT_1
            {"number": 3, "name": "QD"},
            {"number": 4, "name": "QE"},
            {"number": 5, "name": "QF"},
            {"number": 6, "name": "QG"},
            ...
            {"number": 9, "name": "QH'"},
            {"number": 10, "name": "VCC"},
            {"number": 11, "name": "SRCLK"},  # ST_CP
            {"number": 12, "name": "RCLK"},  # SH_CP
            {"number": 13, "name": "SER"},  # DS
            {"number": 14, "name": "OE"},
            {"number": 15, "name": "MR"},
            {"number": 16, "name": "QA"},  # OUTPUT_0
        ]
    },
    "pdfs/74HC595_TI.pdf": {
...
def test_single_pdf(pdf_path, expected, api_key=None):
    ...
    try:
        ...
        for pin in pin_data.pins[:20]:
            print(f"    Pin {pin.number}: {pin.name} - {pin.function or ''}")
        if len(pin_data.pins) > 20:
            print(f"    ... and {len(pin_data.pins) - 20} more")
        ...
        if expected_count:
            if pin_data.package.pin_count == expected_count:
                ...
                print(f"  ❌ Pin count mismatch: expected {expected_count}, got {pin_data.package.pin_count}")
...
def main():
    print("COMPREHENSIVE PIN EXTRACTION TEST")
    print_separator()
